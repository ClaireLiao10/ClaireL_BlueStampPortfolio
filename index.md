...
# Modifications
<iframe width="560" height="315" src="https://www.youtube.com/embed/w_2tAvQqPDs" title="YouTube video player" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>
My modification was to add an LED Matrix to my project. I decided to add an LED Matrix so I can practice my coding skills. I learned about the many functions inside of the MD_Parola Library. One of the functions is the .setTextAlignment() function, in where the user can set where they want to put the text, i.e. left, right, or center. For example: myDisplay.setTextAlignment(PA_LEFT) sets the text to the left. If the user wants to invert the color of the text, the user can use the .setInvert(true) function. The user can control what text the print out using the .print(""). An example can be, if myDisplay wants to print out a text, "Want a piece of candy?" I can write msDisplay.print("Want a piece of candy?"). However, the function that I used for this project was the scroll function. In the scroll function, one can control the speed, the direction it scrolls, and the beginning alignment of the text. Some challenges that I had was finding the perfect wiring for my LED Matrix. The main challenge was to find a wiring that didn't include a breadboard, as the breadboard was too bulky to put in my project. I ended up surfing up the web and finally finding the perfect wiring and code. I also did not have enough space to put in my project, and I solved that issue by building a second story to put my components. In the future, I am hoping to add more modifications to this project such as a weight sensor and possibly buzzer.  
